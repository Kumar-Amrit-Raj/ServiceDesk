...
export async function listTickets(req, res) {
  ...
  const requestedPage = req.query.page ? positiveInteger(req.query.page) : 1;
  const requestedPageSize = req.query.pageSize ? positiveInteger(req.query.pageSize) : 10;
  if (!requestedPage) {
    return res.status(400).json({ message: 'Invalid page number.' });
  }
  if (!requestedPageSize || requestedPageSize > 100) {
  ...
  const { rows: summaryRows } = await pool.query(
    `SELECT
       ...
       COUNT(*) FILTER (WHERE t.status = 'open')::integer AS open,
